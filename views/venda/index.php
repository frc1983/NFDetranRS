...
<section class="card-surface section-card">
    ...
    <?php else: ?><div class="table-responsive"><table class="table"><thead><tr><th>Venda</th><th>Data</th><th>Itens</th><th>NF-e</th><th>Situação</th><th>Ações</th></tr></thead><tbody><?php foreach ($sales as $sale): ?><tr><td><strong><?= Html::encode($sale->numero) ?></strong></td><td><?= Html::encode($sale->created_at) ?></td><td><?= count($sale->items) ?></td><td><?= count($sale->invoices) ?></td><td><span class="badge-soft"><?= Html::encode($sale->status) ?></span></td><td><?php if (Yii::$app->user->can('nfe.emitir')): ?><?= Html::a(count($sale->invoices) ? 'Ver NF-e' : 'Gerar simulação', count($sale->invoices) ? ['/nfe/index'] : ['/nfe/simulate', 'saleId' => $sale->id], count($sale->invoices) ? ['class' => 'btn btn-sm btn-outline-primary'] : ['class' => 'btn btn-sm btn-primary', 'data-method' => 'post', 'data-confirm' => 'Gerar XML e DANFE apenas no simulador local, sem envio a SEFAZ?']) ?><?php endif; ?></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?>
</section>
