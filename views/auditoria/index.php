...
$events = $events ?? [];
$demoEvents = [
    ['event' => 'Estrutura visual inicializada', 'user' => 'Sistema demonstrativo', 'module' => 'Aplicação', 'date' => 'Nesta sessão', 'status' => 'Informativo'],
];
$rows = $events ?: $demoEvents;
?>
...
<section class="metric-grid">
    ...
    <article class="card-surface metric-card"><span class="metric-label">Falhas de integração</span><strong class="metric-value">0</strong><span class="metric-trend">Integrações ainda inativas</span></article>
    <article class="card-surface metric-card"><span class="metric-label">Ações de usuários</span><strong class="metric-value">0</strong><span class="metric-trend">Nenhuma ação operacional</span></article>
...
<form class="card-surface filter-bar" method="get" role="search">
    ...
    <div class="filter-grow"><label class="visually-hidden" for="audit-search">Buscar evento</label><input class="form-control" id="audit-search" name="q" placeholder="Buscar por ação, usuário ou identificador"></div>
    <div><label class="visually-hidden" for="audit-module">Módulo</label><select class="form-select" id="audit-module" name="module"><option value="">Todos os módulos</option><option>GID</option><option>SEFAZ</option><option>Vendas</option><option>Usuários</option></select></div>
    <div><label class="visually-hidden" for="audit-period">Período</label><select class="form-select" id="audit-period" name="period"><option>Hoje</option><option>Últimos 7 dias</option><option>Últimos 30 dias</option></select></div>
    <button class="btn btn-outline-primary" type="button" disabled title="Filtro previsto para a proxima etapa">Aplicar (demo)</button>
</form>
<section class="card-surface section-card">
    <header class="section-head"><div><h2>Eventos registrados</h2><p>Clique em um evento para inspecionar os detalhes.</p></div><?php if (!$events): ?><span class="badge-soft warning">1 exemplo de interface</span><?php endif; ?></header>
